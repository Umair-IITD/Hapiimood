...
export default function AnalyticsPage() {
  ...
  const getMoodEmoji = (score: string) => {
    ...
    if (isNaN(n)) return <Meh className="h-6 w-6 text-text-muted" />;
    if (n >= 4) return <Smile className="h-6 w-6 text-accent-emerald" />;
    if (n >= 2.5) return <Meh className="h-6 w-6 text-accent-amber" />;
    return <Frown className="h-6 w-6 text-accent-rose" />;
  };
                ...
                {sentimentData.length > 0 && (
                  <div className="flex flex-wrap justify-center gap-4 mt-4">
                    {sentimentData.map((d, i) => (
                      <div key={d.name} className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-text-muted">
                        <div className="h-2 w-2 rounded-full" style={{ backgroundColor: SENTIMENT_COLORS[i] }} />
                        {d.name}
                      </div>
                    ))}
                  </div>
